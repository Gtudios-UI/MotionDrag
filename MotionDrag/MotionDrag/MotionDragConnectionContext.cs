...
public class MotionDragConnectionContext<TItem>
{
    ...
    public event MotionDragConnectionDraggingOutside<TItem>? DraggingOutside;
    public event MotionDragConnectionDroppedOutside<TItem>? DroppedOutside;
    ...
    Start:
        if (CurrentReceiver is null)
        {
            foreach (var receiver in Receivers)
            {
                ...
                if (rect.Contains(mousePos) && receiver.IsVisibleAt(mousePos.Point() - receiver.GlobalRectangle.WindowPosOffset))
                {
        ...
        else
        {
            ...
            var rect = globalRect.ContainerRectToScreen;
            if (rect.Contains(mousePos) && CurrentReceiver.IsVisibleAt(mousePos.Point() - CurrentReceiver.GlobalRectangle.WindowPosOffset))
            {
                Point _offset = default;
                CurrentReceiver.DragDelta(sender, item, senderIndex, dragPosition, ref _offset);
                itemOffset += _offset.Point();
            }
            else
            {
